...
class ConditionalUnet1D(nn.Module):
    def __init__(
        ...
        # number in millions
        print("number of parameters: {:.2f}M".format(sum(p.numel() for p in self.parameters()) / 1e6))
...
if __name__ == "__main__":
    ...
    print(
        f"Noise shape = {noise.shape}, Expected shape = [1, {pred_horizon}, {action_dim}]"
    )  # Expected: (1, pred_horizon, action_dim)